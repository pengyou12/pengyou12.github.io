...
function player_death_animation(playerID, x, y) {
	function death_animation() {
		...
		if (mouseposX < x + 65 / 2) {
			...
			$("#player" + playerID + "_death" + imgID).css('transform', 'rotate(' + tempAng + 'deg)');
		}
...
function CheckKey() {
	...
	if (false) //按键冲突 LeftArrow && RightArrow) || (UpArrow && DownArrow)
	{
		...
		if (LeftArrow || RightArrow || UpArrow || DownArrow) {
			...
			if(changeToState == 0)
			{
					$("#player0_0")[0].src = "img/doom_guy.png";
			}
			else {
			$("#player0_0")[0].src = "img/doom_guy_frame" + changeToState + ".png";
		}
		}
	//换图片，实现过渡效果
	}
	$('#player' + 0 + '_' + 0)[0].style.left = player.positionMinx + "px";
	$('#player' + 0 + '_' + 0)[0].style.top = player.positionMiny + "px";
	//$("#player0_" + 0).css('transform', 'rotate(' + angle + 'deg)');
	...
	var enemyGenerate = function() {
		...
		for (var i = 1; i <= e.picNum * 2; i++) {
			...
			if (i == 1 && e.faceto == 0 || i == e.picNum + 1 && e.faceto == 1)
				$("#enemy" + type + "_" + EnemyTotal + "_" + i).css({
					display: "block"
				});
			else
				$("#enemy" + type + "_" + EnemyTotal + "_" + i).css({
