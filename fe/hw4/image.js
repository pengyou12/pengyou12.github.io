...
function setCurrent(value) {
	...
	for (var i = 0; i < sliderItems.length; i++) {
		...
		if (i < current) {
			item.style['-webkit-transform'] = 'translateY(-' + itemWidth * (i + 1) + 'px)';
		} else if (i == current) {
			item.style['-webkit-transform'] = 'translateY(-' + itemWidth * i + 'px)';
		} else {
			item.style['-webkit-transform'] = 'translateY(-' + itemWidth * (i - 1) + 'px)';
		}
	}
...
function myjs() {
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			for (var i = 1; i <= 9; i++) {
				...
				else if (i != 9) myDiv1.append($('<li class= "myli" onclick="changeImageOnFocus(' + (i + 1) + ');" style="display:block"><img onmouseover="cover()" onmouseout="uncover()" onMouseMove="covertrue()" class="myimg" src="' + b + '" "><p class="text">' + c + '</p></li>'));
				else myDiv1.append($('<li class="myli" style="display:block"><img  onmouseover="cover()" onmouseout="uncover()" onMouseMove="covertrue()" class="myimg" src="' + b + '"><p class="text">' + c + '</p></li>'));
			}
			myDiv1.append($('<div class="clearfix"></div>'));
	...
	document.getElementById("currentPage").innerHTML = currentPage;
	//xmlhttp.open("GET", "imageList2.json", true);
	xmlhttp.send();
	preImage = currentImage;
	t2 = window.setInterval("timeChange();", 3000); //定时器跳转
